...
NetworkPort::NetworkPort(const QString &name, PortDirection dir, PortType type, QGraphicsItem *parent)
    : QGraphicsRectItem(parent), m_dir(dir), m_type(type), m_name(name)
{
    QString typeLabel;
    switch (type) {
...
void NetworkPort::addConnection(Connection *conn) { m_connections.append(conn); }
...
void NetworkPort::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    // so that when you drag from a port you dont drag the node around
    event->accept();
}
...
NetworkPort* NetworkNode::addPort(const QString &name, NetworkPort::PortDirection dir, NetworkPort::PortType type)
{
    NetworkPort *port = new NetworkPort(name, dir, type, this);

    if (dir == NetworkPort::Input) {
        port->setPos(rect().left() - port->rect().width() + port->boundingRect().width() - 10, m_label->boundingRect().height() + 5 + m_inputPorts.size() * 28);
        m_inputPorts.append(port);
    } else {
        port->setPos(rect().right() - port->boundingRect().width() + 10 , rect().top() + m_label->boundingRect().height() + 5 + m_outputPorts.size() * 28);
...
void Canvas::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (m_draggingPort && m_tempPath) {
        ...
        if (targetPort)
            connectPorts(m_draggingPort, targetPort);
